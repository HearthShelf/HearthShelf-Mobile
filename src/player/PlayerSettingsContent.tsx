/**
 * Every Player setting, as one reusable block of rows.
 *
 * There are two surfaces onto these settings - the full screen at
 * app/settings/playback.tsx and the in-player More > Player settings sheet
 * (PlayerSettingsSheet) - and they must offer exactly the same controls. They
 * used to be two hand-maintained copies of the same JSX, which drifted: the
 * sheet was missing the player-button editor, mini player, car mode, finishing
 * and queue rows, while the screen was missing "Pick up where you left off".
 * Both now render THIS component, so a row added here shows up in both places
 * and cannot drift again.
 *
 * The only difference between the two is the scroll container, which each
 * caller owns (the screen's SettingsPanel vs. the sheet's BottomSheetScrollView).
 * Navigation rows take an `onNavigate` so the sheet can dismiss itself first.
 */
...
export function PlayerSettingsContent({ onNavigate }: { onNavigate: (href: string) => void }) {
    ...
    <>
      ...
      <SettingsGroup>
        ...
        <SettingsRow
          ...
          desc="Rewind a few seconds when you press play, so a call or a night's sleep doesn't drop you mid-sentence. Longer breaks rewind further."
          control={
            <SettingsToggle on={s.autoRewind} onChange={(v) => setSetting('autoRewind', v)} />
          }
        />
        ...
        <SettingsRow
          ...
          onPress={() => onNavigate('/settings/player-buttons')}
        />
        <SettingsRow
      ...
      <SettingsGroup>
        ...
        {(s.queueMode === 'manual' || s.queueMode === 'auto') && (
          <SettingsRow
            ...
            onPress={() => onNavigate('/settings/queue')}
          />
        )}
