...
- [ ] 1.1 Move `app/(tabs)/more.tsx` to `app/settings/index.tsx`, keeping its
      ...
      longer applies.
- [ ] 1.2 Reduce `app/(tabs)/more.tsx` to a stub that renders nothing - the tab
      stays registered (the tab bars iterate route names) but its press is
      intercepted before the route is reached.
- [ ] 1.3 Repoint every existing `/(tabs)/more` link. Check `app/settings/*`
      back-navigation and any `from=more` params.

## 2. The menu component

- [ ] 2.1 Add `src/ui/MoreMenu.tsx`: a scrim plus an anchored bubble, driven by
      an `open` prop and an `onClose` callback.
- [ ] 2.2 Declare entries as one ordered list of
...
- [ ] 2.3 Set `available` per entry: Discover, Downloads, Settings always;
...
- [ ] 2.4 Add the five feedback-free glyphs the menu needs to `src/ui/icons.ts`
      if absent (Discover reuses `sparkle`; Downloads reuses `download`).
- [ ] 2.5 Style from `theme.ts` tokens - `sheet` ground, `hairline` border,
      `radius.sheet`, `accentWash` for the leading Discover entry. No new
      colour literals.

## 3. Growth animation

- [ ] 3.1 Add a bubble spring config to `src/ui/motion.tsx`. Target the mockup's
      feel: ~520ms to rest, ~2% overshoot, settling back. Do not reuse
      `POP_SPRING` (tuned for press feedback, far too tight for this).
- [ ] 3.2 Animate `scale` from ~0.04 with `transformOrigin` at the bubble's
...
- [ ] 3.3 Stagger the entries fading in from the growth corner outward, so the
      last row appears as the box finishes.
- [ ] 3.4 Honour reduced motion: no growth, no stagger, everything immediately
      visible and interactive.

## 4. Wire it to the tab bar

- [ ] 4.1 In `app/(tabs)/_layout.tsx`, hold the menu's open state and intercept
      `name === 'more'` in `onPressTab` - open the menu instead of navigating.
      Toggle if already open.
- [ ] 4.2 Render `<MoreMenu>` in the tabs layout, outside the scene container so
      it overlays every tab and survives tab switches.
- [ ] 4.3 Pass an open flag into `AppTabBar` so the More tab renders active
      while the menu is up, and set `accessibilityState={{ expanded }}`.
- [ ] 4.4 Anchor per nav mode: above the bar for `classic` and
      `floating-horizontal`; left of the rail for `floating-vertical`, which
      reserves `VNAV_WIDTH` on the right edge.
- [ ] 4.5 Dismiss on scrim tap, on entry tap (then navigate), and on Android
      back via `useBackHandler` returning `true` while open.

## 5. Remove the Discover sparkle button

- [ ] 5.1 Drop the Discover button from `HomeHeader` in `app/(tabs)/index.tsx`,
      leaving arrange, downloads and search.
- [ ] 5.2 Confirm `/discover` is still reachable and that its `from` param
      lights the right tab when opened from the menu.

## 6. Verify

- [ ] 6.1 `npx tsc --noEmit` and `npx prettier --check` on changed files.
